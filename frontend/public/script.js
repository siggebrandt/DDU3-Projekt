...
class CreateQuestion {
  constructor(data) {
    ...
    const allAnswers = [
      this.correct,
      ...data.incorrect_answers.map(this.decode),
    ];
...
quizCategoriesArray.forEach((category) => {
  fetch(`https://api.pexels.com/v1/search?query=${category}&per_page=1`, {
    ...
    .catch((error) => {
      console.error(`Något gick snett med bilden för ${category}`, error);
    });
});
...
async function startQuiz(questions, difficultyChosen) {
  ...
  function showQuestion() {
    let currentQuestions = new CreateQuestion(
      questions.questions[quizProgress]
    );
      ...
      choiceButton.addEventListener("click", function () {
        if (!haveAnswered) {
          ...
          for (let choiceButton of document.querySelectorAll(
            ...
            choiceButton.style.backgroundColor = "#D9534F";
            if (choiceButton.textContent === currentQuestions.correct) {
            ...
            if (quizProgress >= 10) {
              ...
              document.getElementById("resultsOfQuiz").innerHTML = `
                        <div id="quizScore" class="textAlignCenter">Du fick ${correctAnswers} rätt av 10 möjliga!
                            <div id="returnHome" class="flexContainer flexJustifyCenter">
                                <div id="returnHomeButton">Gå tillbaka till startsidan</div>
                            </div>
                        </div>`;
    ...
    .addEventListener("click", async () => {
      ...
      submit.addEventListener("click", async () => {
        if (loggedInUser.username !== username.value) {
          document.querySelector("#delete-status").textContent =
            "You can't delete someone elses account!";
          return;
        }
...
async function profilePicPicker() {
  ...
  let req = new Request(
    `https://api.pexels.com/v1/search?query=${themes[num]}&per_page=5`,
    {
  ...
  for (let img of reso.photos) {
    ...
    div.addEventListener("click", async () => {
      let req = new Request(
        `http://localhost:8000/user/${loggedInUser.id}/profilePic`,
        {
          method: "PATCH",
